...
import PostageDialog from './components/PostageDialog';
import './index.less';
...
export default class Ship extends Component {
  ...
  componentDidMount() {
    this.props.store.pkg.resetShipList();
  ...
  get checkedListLen() {
    return this.props.store.pkg.shipList.filter((item) => item.checked).length;
  }
      ...
      <>
        <ScrollView className="ship" scrollY scrollWithAnimation enhanced pagingEnabled onScrollToLower={this.loadMore}>
          ...
          <View className="select">
            <View className="select-left">
              <CommonCheckbox checked={this.state.allChecked} onChange={this.onChange}></CommonCheckbox>
              全选
            </View>
            <View>
              已选 {this.checkedListLen}/{shipList.length}
            </View>
          </View>
          <View className="list">
            {shipList.slice().map((pkg, index) => (
